...
export default function Transactions() {
  ...
  const renderItem = ({ item: tx }: { item: Transaction }) => {
    ...
    return (
      <TouchableOpacity testID={`tx-item-${tx.id}`} style={s.txRow} activeOpacity={0.7}
        ...
        <View style={s.txInfo}>
          ...
          <Text style={s.txDesc} numberOfLines={1}>{tx.description || formatDate(tx.date)}</Text>
        </View>
        <View style={s.txRight}>
          ...
          <TouchableOpacity testID={`delete-tx-${tx.id}`} onPress={() => handleDelete(tx)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
            <Ionicons name="trash-outline" size={16} color="#D34A3E" />
          </TouchableOpacity>
        </View>
      </TouchableOpacity>
    );
  ...
  return (
    <SafeAreaView style={s.container} testID="transactions-screen">
      {/* Header */}
      <View style={s.header}>
        <Text style={s.title}>Transaksi</Text>
        <TouchableOpacity testID="add-tx-btn" style={s.addBtn} onPress={() => router.push('/add-transaction')}>
      ...
      <View style={s.filterRow}>
        {filters.map(f => (
          <TouchableOpacity key={f.key} testID={`filter-${f.key}`} style={[s.filterPill, filter === f.key && s.filterActive]}
      ...
      {/* List */}
      {loading ? (
        <View style={s.center}><ActivityIndicator size="large" color="#1A4D2E" /></View>
      ) : (
        <FlatList
          data={transactions}
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); loadData(1); }} tintColor="#1A4D2E" />}
          onEndReached={() => { if (page < totalPages && !loadingMore) loadData(page + 1, true); }}
          onEndReachedThreshold={0.3}
          ListFooterComponent={loadingMore ? <ActivityIndicator style={{ padding: 16 }} color="#1A4D2E" /> : null}
          ListEmptyComponent={
            <View style={s.empty}>
              <Ionicons name="receipt-outline" size={56} color="#C2A878" />
              <Text style={s.emptyTitle}>Belum ada transaksi</Text>
              <Text style={s.emptySub}>Tap + untuk menambah transaksi baru</Text>
            </View>
          }
        />
      )}
...
const s = StyleSheet.create({
  ...
  title: { fontSize: 24, fontWeight: '700', color: '#1A4D2E' },
  ...
  filterPill: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#FFF', borderWidth: 1, borderColor: '#F0EBE1' },
  filterActive: { backgroundColor: '#1A4D2E', borderColor: '#1A4D2E' },
  filterText: { fontSize: 13, fontWeight: '500', color: '#7D7D7D' },
  filterTextActive: { color: '#FFF' },
  list: { paddingHorizontal: 20, paddingBottom: 20 },
  txRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: '#F0EBE1' },
  ...
  txDesc: { fontSize: 12, color: '#7D7D7D', marginTop: 2 },
